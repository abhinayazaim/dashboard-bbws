...
class DataBendungan(models.Model):
    """Menyimpan data historis dan input manual harian user"""
    tanggal = models.DateField(unique=True)
    tma = models.FloatField(default=0.0)
    ...
    def __str__(self):
        return f"Data {self.tanggal} - TMA: {self.tma}"
...
class LogPrediksi(models.Model):
    """Menyimpan riwayat hasil prediksi yang pernah dilakukan AI beserta timestamp-nya."""
